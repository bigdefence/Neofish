...
## 기술 면접용 체크리스트 (High-Performance RAG)

- **하이브리드 검색의 원리?** 벡터(의미), 전문(키워드), 단순 매칭을 결합해 검색 누락을 방지함.
- **엔터티 병합 최적화?** 수천 개의 노드를 LLM으로 비교하는 대신, 벡터 유사도로 후보를 선별(Numpy 가속) 후 LLM이 검증함.
- **Neo4j 성능?** Vector Index를 통해 대규모 그래프에서도 밀리초 단위의 시맨틱 조회가 가능함.

**프런트엔드**: Vue 3, Vite 7, Vue Router, **D3**, Axios(`frontend/package.json`). 루트 `npm run setup:backend`는 백엔드에서 **`uv sync`** 실행.
...
| 항목 | 설명 |
|------|------|
| `LLM_API_KEY`, `LLM_BASE_URL` | LLM API(필수 검사) |
| `LLM_MODEL_NAME` | **경량·대량** 호출 기본값: `gemini-3.1-flash-lite-preview`(청크 추출, 프로필, `simulation_config`에 기록되는 **런타임** 모델명 등) |
| `LLM_REASONING_MODEL_NAME` | **고급 추론**: `gemini-3.1-pro-preview`(온톨로지, 시뮬 설정 JSON **생성**, ReportAgent·GraphTools 인스턴스 등) |
| `NEO4J_URI`, `NEO4J_USER`, `NEO4J_PASSWORD` | Bolt(필수) |
| `MAX_CONTENT_LENGTH` | 50MB |
| `UPLOAD_FOLDER` | `backend/uploads` |
| `ALLOWED_EXTENSIONS` | `pdf`, `md`, `txt`, `markdown` |
| `DEFAULT_CHUNK_SIZE` / `DEFAULT_CHUNK_OVERLAP` | 500 / 50 |
| `OASIS_DEFAULT_MAX_ROUNDS` | 기본 10 |
| `OASIS_TWITTER_ACTIONS` / `OASIS_REDDIT_ACTIONS` | 플랫폼별 허용 액션 |
| `REPORT_AGENT_MAX_TOOL_CALLS` | 기본 5 |
| `REPORT_AGENT_MAX_REFLECTION_ROUNDS` | 기본 2 |
| `REPORT_AGENT_TEMPERATURE` | 기본 0.5 |
...
backend/app/
  ...
  models/task.py
  services/
    neo4j_graph_builder.py, neo4j_store.py, neo4j_chunk_extractor.py
    ontology_generator.py, simulation_manager.py
    simulation_config_generator.py, oasis_profile_generator.py
    simulation_runner.py, simulation_ipc.py, graph_memory_updater.py
    graph_tools.py, report_agent.py, podcast_generator.py
backend/scripts/run_twitter_simulation.py, run_reddit_simulation.py, run_parallel_simulation.py
frontend/src/
uploads/projects/
...
| 메서드 | 경로 | 요약 |
|--------|------|------|
| GET | `/project/<project_id>`, `/project/list` | 프로젝트 조회·목록 |
| DELETE | `/project/<project_id>` | 삭제 |
| POST | `/project/<project_id>/reset` | 리셋 |
| POST | `/ontology/generate` | 온톨로지 생성 |
| POST | `/build` | 그래프 비동기 빌드 → `task_id` |
| GET | `/task/<task_id>`, `/tasks` | 작업 상태 |
| GET | `/data/<graph_id>` | 그래프 데이터 |
...
| 메서드 | 경로 | 요약 |
|--------|------|------|
| GET | `/entities/<graph_id>`, `.../by-type/...`, `.../<uuid>` | 엔터티 탐색 |
| POST | `/create` | 시뮬레이션 생성 |
| POST | `/prepare`, `/prepare/status` | 프로필·설정 준비(비동기) |
| GET | `/<simulation_id>`, `/list`, `/history` | 메타·목록 |
| GET | `/<simulation_id>/profiles`, `.../realtime` | 프로필 |
| GET | `/<simulation_id>/config`, `.../realtime`, `.../download` | 설정 JSON |
| POST | `/start`, `/stop` | subprocess 기동/중지 |
| GET | `/<simulation_id>/run-status`, `.../detail` | 실행 상태 |
| GET | `/<simulation_id>/actions`, `/timeline`, `/agent-stats`, `/posts`, `/comments` | 로그·집계 |
| POST | `/interview`, `/interview/batch`, `/interview/all`, `/interview/history` | 인터뷰(IPC) |
| POST | `/env-status`, `/close-env` | 환경 상태 |

### 보고서 `/api/report`

| 메서드 | 경로 | 요약 |
|--------|------|------|
| POST | `/generate`, `/generate/status` | 보고서 생성·상태 |
| GET | `/list`, `/by-simulation/<simulation_id>`, `/<report_id>`, `/<report_id>/download` | 조회·다운로드 |
| DELETE | `/<report_id>` | 삭제 |
| POST | `/chat` | ReportAgent 대화 |
| GET | `/<report_id>/progress`, `/sections`, `/section/<n>` | 진행·섹션 |
| GET | `/check/<simulation_id>` | 시뮬레이션별 보고서 존재 확인 등 |
| GET | `/<report_id>/agent-log`, `.../stream` | 에이전트 로그 |
| GET | `/<report_id>/console-log`, `.../stream` | 콘솔 로그 |
| POST | `/tools/search`, `/tools/statistics` | 그래프 도구 직접 호출 |
| POST | `/<report_id>/podcast/generate` | 팟캐스트 생성(비동기) |
| GET | `/podcast/status/<task_id>` | 팟캐스트 태스크 상태 |
| GET | `/<report_id>/podcast`, `.../podcast/download` | 팟캐스트 메타·다운로드 |

**기타**: `GET /health` → `{ "status": "ok", "service": "NeoFish Backend" }`
...
| 경로 | 설명 |
|------|------|
| `simulation_config.json` | 시간·이벤트·`agent_configs`·`llm_model`(런타임에 사용할 모델명)·`llm_base_url` 등 |
| `twitter_profiles.csv` / `reddit_profiles.json` | OASIS 입력 |
| `run_state.json` | PID·라운드 등 |
| `simulation.log` | subprocess 로그 |
| `ipc_commands/`, `ipc_responses/` | 인터뷰 등 JSON IPC |
| `twitter/actions.jsonl`, `reddit/actions.jsonl` 등 | 액션 스트림 |

### IPC(파일 기반)

`simulation_ipc.py` ↔ `run_*_simulation.py`의 `IPCHandler`.

1. API 서버가 `ipc_commands/<id>.json`에 명령 기록  
2. 시뮬 루프가 폴링 후 실행  
3. `ipc_responses/<id>.json`에 결과  
4. 클라이언트가 수신 후 삭제  

`CommandType`: `interview`, `batch_interview`, `close_env` 등.
