...
class ProjectManager:
    ...
    def list_projects(cls, limit: int = 50) -> List[Project]:
        ...
        for entry in os.listdir(cls.PROJECTS_DIR):
            # 숨김 파일이나 디렉터리가 아닌 항목 건너뜀
            full_path = os.path.join(cls.PROJECTS_DIR, entry)
            if entry.startswith('.') or not os.path.isdir(full_path):
                continue
            try:
                ...
                get_logger('neofish.models.project').warning(
                    f"프로젝트 로드 실패 (건너뜀): {entry} — {e}"
                )
    ...
    @classmethod
    def save_file_bytes_to_project(cls, project_id: str, content: bytes, original_filename: str) -> Dict[str, str]:
        """
        업로드 바이트를 프로젝트 디렉터리에 저장합니다.
