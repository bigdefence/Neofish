"""
시뮬레이션 IPC 통신 모듈.
HTTP API 백엔드와 시뮬레이션 스크립트 간 프로세스 통신에 사용한다.

파일 시스템 기반의 단순 명령/응답 흐름:
1. 백엔드가 `commands/` 디렉터리에 명령을 기록
2. 시뮬레이션 스크립트가 명령을 폴링해 실행 후 `responses/`에 응답 기록
3. 백엔드가 응답 디렉터리를 폴링해 결과 수신
"""
...
logger = get_logger('neofish.simulation_ipc')
