"""
Neo4j 그래프 도구(검색·노드·엣지 조회, Report Agent 연동).
"""
...
from . import neo4j_store as _neo4j_store

logger = get_logger('neofish.graph_tools')
...
class GraphToolsService:
    ...
    - search_graph, get_all_nodes, get_all_edges, ...
    """
    ...
    def __init__(self, llm_client: Optional[LLMClient] = None):
        ...
        logger.info("GraphToolsService 초기화 (Neo4j)")
    ...
    def search_graph(
        ...
        """Neo4j `search_hybrid`로 그래프를 검색한다. 실패 시 로컬 키워드 매칭으로 폴백한다."""
        logger.info(f"그래프 하이브리드 검색: graph_id={graph_id}, query={query[:50]}...")

        try:
            # 하이브리드 검색 (Vector + Full-Text + Keywords)
            facts, edges, nodes = _neo4j_store.search_hybrid(
                graph_id, query, limit
            )
            return SearchResult(
                facts=facts,
                edges=edges,
            ...
            logger.warning("Neo4j 하이브리드 검색 실패, search_simple 폴백: %s", str(e)[:200])
            try:
                facts, edges, nodes = _neo4j_store.search_simple(
                    graph_id, query, limit, scope
                )
                return SearchResult(
                    facts=facts,
                    edges=edges,
                ...
                logger.warning("Neo4j 단순 검색 실패, 로컬 폴백: %s", str(e2)[:200])
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        raw = _neo4j_store.fetch_nodes(graph_id)
        result = [
            NodeInfo(
                uuid=n["uuid"],
                name=n.get("name", ""),
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        raw = _neo4j_store.fetch_edges(graph_id)
        out = []
        for e in raw:
            ei = EdgeInfo(
                uuid=e["uuid"],
    ...
    def get_node_detail(self, node_uuid: str, graph_id: Optional[str] = None) -> Optional[NodeInfo]:
        ...
        if not graph_id:
            return None
        row = _neo4j_store.fetch_node(graph_id, node_uuid)
        if not row:
            return None
        return NodeInfo(
            uuid=row["uuid"],
    ...
    def global_search(self, graph_id: str, query: str) -> Dict[str, Any]:
        """
        커뮤니티 요약(CommunitySummary) 노드들을 조회하여
        전체 그래프 관점의 거시적 질의응답을 수행합니다.
        
        Args:
            graph_id: 그래프 ID
            query: 거시적 관점의 사용자 질문
        """
        logger.info(f"Global Search 실행: {query}")
        
        summaries = _neo4j_store.fetch_community_summaries(graph_id)
        if not summaries:
            return {"query": query, "answer": "그래프에 커뮤니티 요약 정보가 없습니다.", "sources": []}
            
        combined_text = "\n\n".join([f"커뮤니티 {s['uuid']}:\n{s['summary']}" for s in summaries])
        
        system_prompt = "You are an expert graph analyst. Using ONLY the provided community summaries, answer the macro-level question."
        user_prompt = f"Community Summaries:\n{combined_text}\n\nQuestion: {query}\n\nProvide a comprehensive synthesis that addresses the question based on the overarching themes."
        ...
        try:
            ...
            return {"query": query, "answer": "거시적 검색 중 오류가 발생했습니다.", "sources": []}
