"""
Report Agent
LangChain + ReAct 시뮬레이션 보고서 생성

:
1. 시뮬레이션·Neo4j 그래프 정보로 보고서 생성
2. 디렉터리, 생성
3. ReACT
4. , 호출도구
"""
...
logger = get_logger('neofish.report_agent')
...
주의:
- `#`, `##`, `###`, `####` 헤더 금지
- 섹션 제목 문구를 본문 첫 줄·문단으로 다시 쓰지 마세요(화면에 제목이 이미 있음).
- "{section_title}" 전체를 그대로 복사해 넣지 마세요. 근거·분석 문장만 작성하세요.
- 본문 내용만 자연스럽게 작성
"""
...
class ReportAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            graph_tools: Neo4j 그래프 도구 서비스(선택)
        """
        self.graph_id = graph_id
        self.simulation_id = simulation_id
        self.simulation_requirement = simulation_requirement
        
        self.llm = llm_client or LLMClient(model=Config.LLM_REASONING_MODEL_NAME)
        self.graph_tools = graph_tools or GraphToolsService(
            llm_client=LLMClient(model=Config.LLM_REASONING_MODEL_NAME)
        )
    ...
    @staticmethod
    def _extract_final_answer_text(response: Optional[str]) -> str:
        """
        Extract section content from responses that contain a Final Answer marker.

        Some model responses correctly place the marker before the body, while
        others emit the body first and append "Final Answer:" at the end. The
        latter used to produce an empty section because only the trailing slice
        after the marker was kept.
        """
        ...
        first = matches[0]
        after_first = text[first.end():].strip()
        if first.start() == 0 and after_first:
            return after_first

        last = matches[-1]
        after_last = text[last.end():].strip()
        if after_last:
            return after_last

        before_last = text[:last.start()].strip()
        return before_last
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            has_final_answer = bool(re.search(r'Final Answer\s*:', response, re.IGNORECASE))
            ...
            if has_final_answer:
                ...
                # 
                final_answer = self._extract_final_answer_text(response)
        ...
        elif re.search(r'Final Answer\s*:', response, re.IGNORECASE):
            final_answer = self._extract_final_answer_text(response)
        else:
    ...
    def generate_report(
        ...
        try:
            ...
            for i, section in enumerate(outline.sections):
                ...
                cleaned_body = ReportManager._clean_section_content(
                    section_content, section.title
                )
                section.content = cleaned_body
                generated_sections.append(f"## {section.title}\n\n{cleaned_body}")

                # 저장섹션(파일에는 본문만 — UI에 이미 제목이 있어 ## 중복 방지)
                ReportManager.save_section(report_id, section_num, section)
                completed_section_titles.append(section.title)

                # 섹션완료로그(프론트 미리보기용 — 제목 없이 본문만)
                if self.report_logger:
                    self.report_logger.log_section_full_complete(
                        section_title=section.title,
                        section_index=section_num,
                        full_content=cleaned_body or "",
                    )
...
class ReportManager:
    ...
    _report_index_cache: Optional[Dict[str, Any]] = None
    _line_offsets_cache: Dict[str, Dict[str, Any]] = {}
    ...
    @classmethod
    def _get_report_index_path(cls) -> str:
        """蹂닿퀬?쒖씤?깅뙆??"""
        return os.path.join(cls.REPORTS_DIR, "_index.json")
    ...
    @classmethod
    def _make_report_index_entry(cls, report: Any, report_id_hint: Optional[str] = None) -> Dict[str, Any]:
        """리포트 객체/딕셔너리를 인덱스 엔트리로 변환"""
        if isinstance(report, Report):
            return {
                ...
                "completed_at": report.completed_at or "",
            }

        return {
            "report_id": report.get("report_id") or report_id_hint or "",
            "simulation_id": report.get("simulation_id") or "",
            "graph_id": report.get("graph_id") or "",
    ...
    @classmethod
    def _save_report_index(cls, index_data: Dict[str, Any]) -> Dict[str, Any]:
        """보고서 인덱스를 저장하고 메모리 캐시를 갱신"""
        cls._ensure_reports_dir()
        with open(cls._get_report_index_path(), 'w', encoding='utf-8') as f:
            json.dump(index_data, f, ensure_ascii=False, indent=2)

        cls._report_index_cache = index_data
        return index_data

    @classmethod
    def _rebuild_report_index(cls) -> Dict[str, Any]:
        """디스크 기준으로 인덱스를 재구성"""
        cls._ensure_reports_dir()
        ...
        index_data = {
            "version": 1,
            "reports": {},
            "latest_by_simulation": {},
        }

        for item in os.listdir(cls.REPORTS_DIR):
            if item == "_index.json" or item.startswith('.'):
                continue
            ...
            entry = cls._make_report_index_entry(meta, report_id_hint=report_id_hint)
            report_id = entry.get("report_id")
            simulation_id = entry.get("simulation_id")
            if not report_id or not simulation_id:
                continue

            index_data["reports"][report_id] = entry

            current_latest = index_data["latest_by_simulation"].get(simulation_id)
            current_entry = index_data["reports"].get(current_latest) if current_latest else None
            if current_entry is None or entry.get("created_at", "") >= current_entry.get("created_at", ""):
                index_data["latest_by_simulation"][simulation_id] = report_id

        return cls._save_report_index(index_data)

    @classmethod
    def _load_report_index(cls, force_refresh: bool = False) -> Dict[str, Any]:
        """보고서 인덱스를 로드"""
        if cls._report_index_cache is not None and not force_refresh:
            return cls._report_index_cache

        index_path = cls._get_report_index_path()
        if not force_refresh and os.path.exists(index_path):
            try:
                ...
                if (
                    ...
                    and index_data.get("version") == 1
                    and isinstance(index_data.get("reports"), dict)
                    and isinstance(index_data.get("latest_by_simulation"), dict)
                ):
                    cls._report_index_cache = index_data
                    return index_data
            except (OSError, json.JSONDecodeError):
        ...
        return cls._rebuild_report_index()

    @classmethod
    def _upsert_report_index_entry(cls, report: Any) -> None:
        """인덱스에 리포트 메타를 반영"""
        entry = cls._make_report_index_entry(report)
        report_id = entry.get("report_id")
        simulation_id = entry.get("simulation_id")
        if not report_id or not simulation_id:
            return

        index_data = cls._load_report_index()
        index_data["reports"][report_id] = entry
        ...
        cls._save_report_index(index_data)

    @classmethod
    def _remove_report_index_entry(cls, report_id: str) -> None:
        """인덱스에서 리포트 메타를 제거"""
        index_data = cls._load_report_index()
        entry = index_data["reports"].pop(report_id, None)
        if entry is None:
            return

        simulation_id = entry.get("simulation_id")
        if simulation_id and index_data["latest_by_simulation"].get(simulation_id) == report_id:
            remaining = [
                rid
                for rid, meta in index_data["reports"].items()
                if meta.get("simulation_id") == simulation_id
            ]
            if remaining:
                remaining.sort(
                    key=lambda rid: index_data["reports"][rid].get("created_at", ""),
                    reverse=True,
                )
                index_data["latest_by_simulation"][simulation_id] = remaining[0]
            else:
                index_data["latest_by_simulation"].pop(simulation_id, None)

        cls._save_report_index(index_data)

    @classmethod
    def _get_sorted_report_ids(
        cls,
        ...
        """인덱스 기준으로 리포트 ID를 정렬"""
        index_data = cls._load_report_index()
        report_items = index_data["reports"].items()

        if simulation_id is not None:
            report_ids = [
                report_id
                for report_id, meta in report_items
                if meta.get("simulation_id") == simulation_id
            ]
        else:
            report_ids = [report_id for report_id, _ in report_items]

        report_ids.sort(
            key=lambda report_id: index_data["reports"][report_id].get("created_at", ""),
            reverse=True,
        )

        if limit is not None:
            return report_ids[:limit]
        return report_ids

    @classmethod
    def _clear_report_file_caches(cls, report_id: str) -> None:
        """로그/섹션 관련 캐시를 초기화"""
        folder = cls._get_report_folder(report_id)
        stale_paths = [
            path
            for path in cls._line_offsets_cache
            if path.startswith(folder)
        ]
        for path in stale_paths:
            cls._line_offsets_cache.pop(path, None)

        cls._sections_cache.pop(report_id, None)

    @classmethod
    def _get_file_line_offsets(cls, file_path: str) -> List[int]:
        """텍스트 파일의 라인 시작 오프셋을 캐시"""
        stat = os.stat(file_path)
        mtime_ns = getattr(stat, "st_mtime_ns", int(stat.st_mtime * 1_000_000_000))
        cache = cls._line_offsets_cache.get(file_path)

        if cache and cache.get("size") == stat.st_size and cache.get("mtime_ns") == mtime_ns:
            return cache["offsets"]

        if cache and stat.st_size >= cache.get("size", 0):
            offsets = list(cache.get("offsets", [0]))
            start_offset = cache.get("size", 0)
        else:
            offsets = [0]
            start_offset = 0
        ...
        cls._line_offsets_cache[file_path] = {
            "size": stat.st_size,
            "mtime_ns": mtime_ns,
            "offsets": offsets,
        }
    ...
    @classmethod
    def _read_text_lines_from(cls, file_path: str, from_line: int) -> Dict[str, Any]:
        """라인 오프셋 캐시를 이용해 필요한 구간만 읽기"""
        offsets = cls._get_file_line_offsets(file_path)
        total_lines = max(len(offsets) - 1, 0)
        start_line = max(from_line, 0)
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        """Return console log lines starting at ``from_line``."""
        ...
        log_data = cls._read_text_lines_from(log_path, from_line)
        return {
            "logs": log_data["lines"],
    ...
    def get_agent_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        """Return structured agent log entries starting at ``from_line``."""
        ...
        raw_data = cls._read_text_lines_from(log_path, from_line)
        logs = []
    ...
    def save_section(
        ...
        # 섹션 파일에는 본문만 저장(제목은 outline + assemble_full_report에서 부여)
        file_suffix = f"section_{section_index:02d}.md"
        file_path = os.path.join(cls._get_report_folder(report_id), file_suffix)
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(cleaned_content if cleaned_content else "")
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        joined = '\n'.join(cleaned_lines)
        return cls._strip_leading_title_duplicate(joined, section_title)
    ...
    def _titles_match(a: str, b: str) -> bool:
        if not a or not b:
            return False
        x, y = a.strip(), b.strip()
        return x == y or x.replace(' ', '') == y.replace(' ', '')

    @classmethod
    def _strip_leading_title_duplicate(cls, text: str, section_title: str) -> str:
        """
        본문 앞부분에 섹션 제목과 동일한 줄(일반 텍스트, ##, **제목**)이 있으면 제거.
        UI·assemble_full_report에서 제목을 따로 붙이므로 중복 표시를 막음.
        """
        if not text or not section_title:
            return (text or '').strip()

        s = text.strip()
        # 선행 Markdown 헤더(금지 규칙 위반 시)
        while s:
            first_line = s.split('\n', 1)[0].strip()
            m = re.match(r'^(#{1,6})\s+(.+)$', first_line)
            if m and cls._titles_match(m.group(2).strip(), section_title):
                s = s.split('\n', 1)[1].strip() if '\n' in s else ''
                continue
            break
        ...
        for _ in range(max_passes):
            ...
            if not lines:
                break
            first = lines[0].strip()
            if cls._titles_match(first, section_title):
                lines.pop(0)
                continue
            bold_m = re.match(r'^\*\*(.+?)\*\*$', first)
            if bold_m and cls._titles_match(bold_m.group(1).strip(), section_title):
                lines.pop(0)
                continue
            break
    ...
    @classmethod
    def _strip_legacy_section_file_heading(cls, raw: str, section_title: str) -> str:
        """과거 형식(파일 첫 줄에 ## 제목) 호환."""
        if not raw or not section_title:
            return (raw or '').strip()
        s = raw.strip()
        m = re.match(r'^#{1,6}\s+(.+?)\s*\n+', s)
        if m and cls._titles_match(m.group(1).strip(), section_title):
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        cache_key = tuple(
            (item["filename"], item["size"], item["mtime_ns"])
            for item in section_files
        )
        cached = cls._sections_cache.get(report_id)
        if cached and cached.get("key") == cache_key:
            return list(cached["sections"])
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # 읽기섹션파일 — outline 제목으로 ## 부여(본문 파일은 제목 없이 저장됨)
        sections = sorted(
            cls.get_generated_sections(report_id),
        ...
        for i, section_info in enumerate(sections):
            title = (
                ...
                else f"섹션 {section_info['section_index']}"
            )
            raw_body = cls._strip_legacy_section_file_heading(
                section_info['content'], title
            )
            body = cls._clean_section_content(raw_body, title)
            md_content += f"## {title}\n\n"
            if body:
                md_content += f"{body}\n\n"
    ...
    def save_report(cls, report: Report) -> None:
        ...
        cls._upsert_report_index_entry(report)
    ...
    @classmethod
    def get_report_id_by_simulation(cls, simulation_id: str) -> Optional[str]:
        """Return the newest report id for a simulation."""
        index_data = cls._load_report_index()
        report_id = index_data["latest_by_simulation"].get(simulation_id)
        if report_id and os.path.exists(cls._get_report_path(report_id)):
            return report_id

        index_data = cls._load_report_index(force_refresh=True)
        report_id = index_data["latest_by_simulation"].get(simulation_id)
        if report_id and os.path.exists(cls._get_report_path(report_id)):
            return report_id
        return None
    ...
    def get_report_by_simulation(cls, simulation_id: str) -> Optional[Report]:
        """Return the newest report for a simulation."""
        report_id = cls.get_report_id_by_simulation(simulation_id)
        if not report_id:
            return None
    ...
    def list_reports(cls, simulation_id: Optional[str] = None, limit: int = 50) -> List[Report]:
        ...
        reports = []
        for report_id in cls._get_sorted_report_ids(simulation_id=simulation_id, limit=limit):
    ...
    def delete_report(cls, report_id: str) -> bool:
        """Delete a report and invalidate related caches."""
        import shutil
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            ...
            cls._clear_report_file_caches(report_id)
            cls._remove_report_index_entry(report_id)
