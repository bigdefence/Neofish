"""
OASIS Agent Profile 생성
Neo4j 그래프 엔터티를 바탕으로 OASIS 시뮬레이션용 Agent Profile을 만든다.
"""
...
from .neo4j_store import search_simple

logger = get_logger('neofish.oasis_profile')
...
class OasisAgentProfile:
    ...
    institutional_psychology: Optional[str] = None
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        if self.psychological_profile:
            profile["psychological_profile"] = self.psychological_profile
        if self.cognitive_biases:
            profile["cognitive_biases"] = self.cognitive_biases
        if self.institutional_psychology:
            profile["institutional_psychology"] = self.institutional_psychology
        if self.biases:
            profile["biases"] = self.biases
    ...
    def to_twitter_format(self) -> Dict[str, Any]:
        ...
        # 심리적 정보 추가 (Twitter user_char에 통합 권장)
        if self.psychological_profile:
            profile["psychological_profile"] = self.psychological_profile
        if self.cognitive_biases:
            profile["cognitive_biases"] = self.cognitive_biases
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d = {
            ...
            "institutional_psychology": self.institutional_psychology,
            "biases": self.biases,
...
class OasisProfileGenerator:
    """Neo4j 그래프 엔터티로 OASIS Agent Profile을 생성한다."""
    ...
    def __init__(
        ...
        self.verbose_profile_logging = Config.OASIS_PROFILE_VERBOSE_LOGGING
        self.realtime_save_every = max(1, Config.OASIS_PROFILE_REALTIME_SAVE_EVERY)
        self.realtime_save_min_interval_seconds = max(
            0.0,
            Config.OASIS_PROFILE_REALTIME_SAVE_MIN_INTERVAL_SECONDS,
        )
        self._graph_context_cache: Dict[str, Dict[str, Any]] = {}
        self._graph_context_cache_lock = threading.Lock()
    ...
    def generate_profile_from_entity(
        ...
        """
        엔터티로 OASIS Agent Profile을 생성한다.
        ...
        return OasisAgentProfile(
            ...
            # 심리적 필드 맵핑
            psychological_profile=profile_data.get("psychological_profile") or profile_data.get("institutional_psychology"),
            cognitive_biases=profile_data.get("cognitive_biases") or profile_data.get("biases"),
            institutional_psychology=profile_data.get("institutional_psychology"),
            biases=profile_data.get("biases"),
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """Neo4j `search_simple`로 엔터티 주변 맥락을 보강한다."""
        ...
        try:
            ...
            results["context"] = "\n\n".join(context_parts)
            logger.info(
                "Neo4j 검색 완료: %s, %s건 사실, %s개 노드",
                entity_name,
                len(results["facts"]),
                len(results["node_summaries"]),
            )
        except Exception as e:
            logger.warning("Neo4j 검색 실패 (%s): %s", entity_name, e)
...
2. persona: 2000자 이내의 상세 성격/행동 특성. 이 인물의 가치관, 심리적 배경(psychological_profile), 그리고 취약한 인지 편향(cognitive_biases)을 반영하여 사회적 상황에서 어떻게 반응할지 구체적으로 기술하세요.
3. age: 정수
...
9. psychological_profile: 엔터티 속성에서 추출된 심리적 배경 요약
10. cognitive_biases: 이 인물이 가진 주요 인지 편향 목록 (예: "확증 편향, 손실 회피")
...
반드시 아래 키를 포함한 JSON만 반환하세요:
1. bio: 200자 이내 소개
2. persona: 2000자 이내 상세 설명. 기관의 목표, 의사결정 방식(institutional_psychology), 그리고 조직 내 편향(biases)을 반영하세요.
3. age: 30 (고정)
4. gender: "other" (고정)
5. mbti: "ISTJ" (기본값)
6. country: 국가명 문자열
...
8. interested_topics: 관심 주제 문자열 배열
9. institutional_psychology: 조직 문화 및 의사결정 성향
10. biases: 조직적 인지 편향 (예: "매몰 비용 오류, 집단 사고")
...
"""
    ...
    def set_graph_id(self, graph_id: str):
        """프로필 생성에 사용할 Neo4j graph_id를 설정한다."""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        total = len(entities)
        parallel_count = max(1, min(parallel_count, total))
        ...
        def save_profiles_realtime(force: bool = False):
            ...
            with lock:
                ...
                now = time.monotonic()
                current_count = len(existing_profiles)
                should_save = force or current_count == total
                if not should_save:
                    enough_profiles = (
                        current_count - last_saved_count[0]
                    ) >= self.realtime_save_every
                    enough_time = (
                        self.realtime_save_min_interval_seconds == 0.0
                        or (now - last_save_at[0]) >= self.realtime_save_min_interval_seconds
                    )
                    should_save = enough_profiles and enough_time
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # 생성콘솔로그
                if self.verbose_profile_logging:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # 쓰기파일
                    if current == total or current - last_saved_count[0] >= self.realtime_save_every:
                        save_profiles_realtime()
                        last_saved_count[0] = current
        ...
        if realtime_output_path and last_saved_count[0] < len([p for p in profiles if p is not None]):
