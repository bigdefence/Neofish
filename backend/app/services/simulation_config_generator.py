...
from .entity_types import EntityNode

logger = get_logger('neofish.simulation_config')
...
class AgentActivityConfig:
    ...
    # 심리학적 요인 추가
    conformity_level: float = 0.5        # 군중 동조성 (0.0~1.0)
    emotional_reactivity: float = 0.5    # 감정적 반응성 (0.0~1.0)
    cognitive_biases: List[str] = field(default_factory=list) # 취약한 인지 편향
...
@dataclass
class WorldPsychologyConfig:
    """전역적 심리 환경 설정"""
    # 전반적인 군중 심리 민감도 (0.0~1.0)
    global_conformity_pressure: float = 0.5
    # 감정 전염 속도 (0.0~1.0)
    emotional_contagion_rate: float = 0.5
    # 정보 폭포 효과 발생 확률 (0.0~1.0)
    information_cascade_probability: float = 0.3
    # 집단적 공포/낙관 성향 (-1.0~1.0)
    global_mood_bias: float = 0.0
...
class SimulationParameters:
    ...
    # 전역 심리 설정 추가
    world_psychology: WorldPsychologyConfig = field(default_factory=WorldPsychologyConfig)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "world_psychology": asdict(self.world_psychology),
...
class SimulationConfigGenerator:
    ...
    def __init__(
        ...
        # 설정 JSON 생성(시간·이벤트·에이전트 배치 등) — 고급 추론
        self.model_name = model_name or Config.LLM_REASONING_MODEL_NAME
        # simulation_config에 저장되어 OASIS가 사용하는 모델 — 대량 호출이므로 경량 모델
        self.runtime_llm_model = runtime_llm_model or Config.LLM_MODEL_NAME
    ...
    def generate_config(
        ...
        total_steps = 4 + num_batches  # 시간 + 심리 + 이벤트 + NAgent + 플랫폼
        current_step = 0
        ...
        reasoning_parts.append(f"시간설정: {time_config_result.get('reasoning', '')}")
        
        # ========== 2: 생성 전역 심리 설정 ==========
        report_progress(2, "생성 전역 심리 환경 설정...")
        psych_result = self._generate_world_psychology_config(context, simulation_requirement)
        world_psychology = self._parse_world_psychology_config(psych_result)
        reasoning_parts.append(f"심리설정: {psych_result.get('reasoning', '')}")
        
        # ========== 3: 생성설정 ==========
        report_progress(3, "생성설정...")
        event_config_result = self._generate_event_config(context, simulation_requirement, entities)
        event_config = self._parse_event_config(event_config_result)
        reasoning_parts.append(f"이벤트설정: {event_config_result.get('reasoning', '')}")
        ...
        params = SimulationParameters(
            ...
            llm_model=self.runtime_llm_model,
    ...
    def _generate_world_psychology_config(self, context: str, req: str) -> Dict[str, Any]:
        """시뮬레이션 전역 심리 설정을 생성한다."""
        prompt = f"""다음 시뮬레이션 요구사항과 문맥을 분석하여 전역 심리 환경 설정 JSON을 생성하세요.

요구사항: {req}
문맥: {context[:5000]}

## 작업
군중 심리, 감정 전염, 정보 전파 특성을 정의하는 `world_psychology`를 작성하세요.

## 필드 가이드
- global_conformity_pressure: 주변 의견에 동조하려는 압력 세기 (0.0~1.0)
- emotional_contagion_rate: 감정이 한 에이전트에서 다른 에이전트로 퍼지는 속도 (0.0~1.0)
- information_cascade_probability: 논리보다 다수의 행동을 따라가는 '폭포 효과' 발생 확률 (0.0~1.0)
- global_mood_bias: 시뮬레이션 시작 시점의 집단적 분위기 (-1.0: 매우 부정/공포, 1.0: 매우 긍정/낙관)

{{
    "global_conformity_pressure": 0.6,
    "emotional_contagion_rate": 0.5,
    "information_cascade_probability": 0.4,
    "global_mood_bias": -0.2,
    "reasoning": "설정 근거"
}}"""
        system_prompt = "당신은 사회 심리학 및 시뮬레이션 설계 전문가입니다. JSON만 반환하세요."
        try:
            return self._call_llm_with_retry(prompt, system_prompt)
        except Exception as e:
            logger.warning(f"심리 설정 생성 실패: {e}. 기본값 사용.")
            return {"reasoning": "기본 심리 설정 적용"}

    def _parse_world_psychology_config(self, result: Dict[str, Any]) -> WorldPsychologyConfig:
        """LLM 결과를 WorldPsychologyConfig로 변환한다."""
        return WorldPsychologyConfig(
            global_conformity_pressure=result.get("global_conformity_pressure", 0.5),
            emotional_contagion_rate=result.get("emotional_contagion_rate", 0.5),
            information_cascade_probability=result.get("information_cascade_probability", 0.3),
            global_mood_bias=result.get("global_mood_bias", 0.0)
        )
...
- conformity_level(0.0~1.0): 주변 의견에 얼마나 쉽게 동조하는가
- emotional_reactivity(0.0~1.0): 자극적인 정보에 대한 감정적 반응 강도
- cognitive_biases(문자열 배열): 이 엔터티가 가질만한 인지 편향 (예: ["Confirmation Bias", "Loss Aversion"])

권장 가이드:
- University/GovernmentAgency: 활동 낮음, 응답 느림, 영향력 높음, 동조성 낮음
- MediaOutlet: 활동 높음, 응답 빠름, 영향력 중상, 중립적 태도
- Student/Person/Alumni: 활동 중간~높음, 응답 빠름, 영향력 중간, 동조성 상대적 높음
...
{{
    "agent_configs": [
        {{
            ...
            "stance": "<supportive/opposing/neutral/observer>",
            "influence_weight": <>,
            "conformity_level": <0.0-1.0>,
            "emotional_reactivity": <0.0-1.0>,
        ...
        for i, entity in enumerate(entities):
            ...
            config = AgentActivityConfig(
                ...
                conformity_level=cfg.get("conformity_level", 0.5),
                emotional_reactivity=cfg.get("emotional_reactivity", 0.5),
                cognitive_biases=cfg.get("cognitive_biases", [])
            )
