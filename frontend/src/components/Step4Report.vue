...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const factMatch = text.match(/(?:관련 예측\s*)?사실:\s*(\d+)/)
    const entityMatch = text.match(/(?:관련\s*)?엔터티:\s*(\d+)/)
    const relMatch = text.match(/관계 체인:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    const subQSection = text.match(/### 분석 하위 질문[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }

    const factsSection = text.match(/### \[(?:핵심\s*)?사실\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    const entitySection = text.match(/### \[(?:핵심\s*)?엔터티\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/사실:\s*(\d+)/)
        return {
    ...
    const relSection = text.match(/### \[관계 체인\][\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/현재\s*유효\s*사실:\s*(\d+)/)
    const histMatch = text.match(/과거\s*\/\s*만료\s*사실:\s*(\d+)/)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryLine = lines.find(line => line.includes(':') && !line.trim().startsWith('#'))
    if (queryLine) {
      result.query = queryLine.replace(/^[^:]+:\s*/, '').trim()
    }

    const statsLine = lines.find((line, index) => index <= 2 && /\d/.test(line) && !line.includes(':'))
    const countMatch =
      statsLine?.match(/(\d+)/) ||
      normalized.match(/(?:found|results?)\s*[:=]?\s*(\d+)/i) ||
      normalized.match(/(\d+)\s*results?/i)
    ...
    const parseNode = (line) => {
      ...
      if (richMatch) return { name: richMatch[1].trim(), type: richMatch[2].trim() }

      const typedMatch = line.match(/^-\s*(.+?)\s*\((.+?)\)\s*$/)
      if (typedMatch) return { name: typedMatch[1].trim(), type: typedMatch[2].trim() }
    ...
    for (const section of sections) {
      const bodyLines = section.body.map(line => line.trim()).filter(Boolean)
      if (bodyLines.length === 0) continue

      const numberedFacts = bodyLines
        .filter(line => /^\d+\.\s+/.test(line))
        .map(line => line.replace(/^\d+\.\s*/, '').trim())
        .filter(Boolean)

      if (numberedFacts.length > 0) {
        result.facts.push(...numberedFacts)
        continue
      }

      const parsedEdges = bodyLines
        .filter(line => line.startsWith('-'))
        .map(parseEdge)
        .filter(Boolean)

      if (parsedEdges.length > 0) {
        result.edges.push(...parsedEdges)
        continue
      }

      const parsedNodes = bodyLines
        .filter(line => line.startsWith('-'))
        .map(parseNode)
        .filter(Boolean)

      if (parsedNodes.length > 0) {
        result.nodes.push(...parsedNodes)
      }
    }
    ...
    if (result.count === 0) {
      result.count = result.facts.length || result.edges.length || result.nodes.length
    }
  } catch (e) {
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid transparent; padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--text-muted);
  --wf-divider: transparent;

  --wf-active-bg: transparent;
  --wf-active-border: var(--foreground);
  --wf-active-dot: var(--foreground);
  --wf-active-text: var(--foreground);

  --wf-done-bg: transparent;
  --wf-done-border: var(--text-muted);
...
.tool-badge.tool-orange {
  background: linear-gradient(135deg, var(--card)7ED 0%, #FFEDD5 100%);
  border-color: #FDBA74;
  color: #C2410C;
}
