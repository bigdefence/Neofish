          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">Neofish가 현실 시간 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span>시간을 자동 계획합니다. 라운드 1회는 현실 시간 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span>분에 해당합니다.</span>
              </div>
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`그래프에서 ${res.data.expected_entities_count}개 엔티티를 확인했습니다.`)
...
.badge.success { background: rgba(0, 240, 255, 0.1); color: var(--accent); border: 1px solid var(--accent); text-shadow: var(--accent-glow); }
.badge.processing { background: var(--warning); color: var(--card); }
.badge.pending { background: rgba(0,0,0,0.5); color: var(--text-muted); border: 1px solid var(--text-muted); }
.badge.accent { background: rgba(255, 0, 85, 0.1); color: var(--warning); border: 1px solid var(--warning); text-shadow: 0 0 5px var(--warning); }
