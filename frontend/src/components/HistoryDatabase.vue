<template>
  ...
  >
    ...
    <div v-else-if="loadError" class="history-empty-state history-error">
      <p class="history-empty-title">기록을 불러오지 못했습니다</p>
      <p class="history-empty-desc">{{ loadError }}</p>
      <button type="button" class="history-retry-btn" @click="loadHistory">다시 시도</button>
    </div>

    <div v-else-if="projects.length === 0" class="history-empty-state">
      <p class="history-empty-title">저장된 시뮬레이션이 없습니다</p>
      <p class="history-empty-desc">시뮬레이션을 실행하면 이곳에 목록이 표시됩니다. (서버의 uploads/simulations 기준)</p>
    </div>
...
const loadHistory = async () => {
  try {
    ...
    const msg =
      ...
      error?.message ||
      '네트워크 오류이거나 백엔드(기본 5001)가 실행 중인지 확인하세요.'
    loadError.value = typeof msg === 'string' ? msg : JSON.stringify(msg)
  } finally {
    loading.value = false
  }
}
...
.history-error .history-empty-title {
  color: var(--warning, #ff2a2a);
}
